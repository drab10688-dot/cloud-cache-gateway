import { Cloud, ExternalLink, Copy, Power, PowerOff, Loader2, Terminal, CheckCircle } from "lucide-react";
  ...
  const toggleTunnel = async () => {
    ...
    try {
      if (tunnelActive) {
        ...
        // Retry fetching URL if not immediately available
        if (!result.url) {
  ...
  return (
    <div>
      ...
      <div className={`rounded-lg p-6 mb-6 border-2 transition-all duration-500 ${
        tunnelActive ? "border-success/50 bg-success/5 glow-success" : "border-border bg-card card-glow"
      }`}>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {tunnelActive ? "Tu panel es accesible desde cualquier lugar" : "Presiona para generar un link de acceso público"}
              </p>
            </div>
          </div>
          <Button onClick={toggleTunnel} disabled={loading} size="lg"
